...
set <int> getPossibleMoves(const char* board, int location)
{
   ...
   if (board[location] == 'P')
   {
      ...
      if (row == 6 && board[r * 8 + c] == ' ')
          // only if not about to be captured -- this probably works but solves wrong problem 
          // if ((board[(r-2) * 8 + (c+1)] != 'p') && (board[(r-2) * 8 + (c-1)] == 'p'))
      ...
      // en-passant:
      // if (row == 4 && board[row * 8 + (col + 1)] == 'p') // or -1 // who just moved there.. how do we freaking know that? 

      // char board[64] = { BOARD FOR REFERENCE 
      //    'r', 'n', 'b', 'q', 'k', 'b', 'n', 'r',
      //    'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p',
      //    ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ',
      //    ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ',
      ...
      // if (r == 1)
            // board[location] == 'Q';
      // if ((board[positionTo] == 'P' || board[positionTo] == 'p') && (row == 0 || row == 7))
           // board[location] == 'Q';?
...
bool move(char* board, int positionFrom, int positionTo)
{
   ...
   if (possiblePrevious.find(positionTo) != possiblePrevious.end())
   {
      ...
      //int row = positionTo / 8;  // row of position moved to
      //int col = positionTo % 8;  // column of position moved to
      // if ((board[positionTo] == 'P') && (row == 0))
        // board[positionTo] = 'Q'; 
      // if ((board[positionTo] == 'p') && (row == 7))
        // board[positionTo] = 'q'; 
      return true;
   }
